...
class GetNotePageInput(unittest.TestCase):
    ...
    def testCase01_GetNotePageFailed(self):
        ...
        rows = -1
        user_id2 = "336928044"
        path = f'/v3/notesvr/user/{user_id2}/home/startindex/{startindex}/rows/{rows}/notes'
    ...
    def testCase02_GetNotePageFailed(self):
        ...
        startindex = 0
        rows = -1
        invalid_user_id = DataGenerator.generate_time_str() + "userid"
        path = f'/v3/notesvr/user/{invalid_user_id}/home/startindex/{startindex}/rows/{rows}/notes'
        res = get(url=self.host + path, sid=self.sid1)
        self.assertEqual(412, res.status_code)
        expect = {"errorCode": -1011, "errorMsg": "user change!"}
        CheckOutput().output_check(expect=expect, actual=res.json())
    ...
    def testCase03_GetNotePageFailed(self):
        ...
        res = get(url=self.host + path, sid=invalid_wps_id)
    ...
    def testCase08_GetNotePageFailed(self):
        """获取首页便签：rows值为越界值校验"""
        step("前置构建1条便签数据")
        DataCreate().create_note(num=1)
        step("获取首页便签")
